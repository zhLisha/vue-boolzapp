var app = new Vue(
	{
		...
		methods: {
			...
			// Ricerca del contatto attraverso la digitazione dei singoli caratteri
			inputFilterContact() {
				// Trasformo tutti i carateri inseriti dall'utente in minuscolo
				const lowerUserFilter = this.filterContacts.toLowerCase();

				//Se i caratteri inseriti dall'utente sono presenti nell'array contacts, allora verranno mostrati solo quest'ultimi, altrimenti la lista rimane invariata
				this.contacts.forEach((element) => {
					// Trasformo anche i caratteri dei contatti in minuscolo per evitare disuguaglianze tra maiuscolo e minuscolo
					const lowerContactFilter = element.name.toLowerCase();

					if(lowerContactFilter.includes(lowerUserFilter)) {
